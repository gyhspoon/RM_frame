...
MotorINFO GMP  = Gimbal_MOTORINFO_Init(1.0,&ControlGMP,
									   ...
									   fw_PID_INIT(6000,15,0, 	10000.0, 10000.0, 10000.0, 29000.0));
MotorINFO GMY  = Gimbal_MOTORINFO_Init(1.0,&ControlGMY,
									   fw_PID_INIT(0.5,0,0, 	10.0, 10.0, 10.0, 0),
									   fw_PID_INIT(500,3,0, 	10000.0, 10000.0, 10000.0, 5000.0));
...
void ControlGMY(MotorINFO* id)
{
	...
	{		
		...
		else id->positionPID.outputMax = 5.0;
		id->Intensity = PID_PROCESS_Double(&(id->positionPID),&(id->speedPID),id->TargetAngle,id->RealAngle,-ThisSpeed);
...
void ControlGMP(MotorINFO* id)
{
	...
	{		
		...
		#endif
		#ifdef GM_TEST
		MINMAX(id->TargetAngle, id->RealAngle - (GM_PITCH_ZERO - id->RxMsg6623.angle) * 360.0 / 8192.0 / id->ReductionRate - 35.0f, id->RealAngle - (GM_PITCH_ZERO - id->RxMsg6623.angle) * 360.0 / 8192.0 / id->ReductionRate + 35.0f);
		#endif
